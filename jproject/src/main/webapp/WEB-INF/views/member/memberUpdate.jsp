							...
							<input type="radio" name="gender" id="female" value="여자" checked>여자
								...
								<tr>
									...
									<td><select name="age" id="age">
											...
											<option value="10"
												<c:if test="${edit.age == '10'}"> ${'selected'} </c:if>>10대</option>
											<option value="20"
												<c:if test="${edit.age == '20'}"> ${'selected'} </c:if>>20대</option>
											<option value="30"
												<c:if test="${edit.age == '30'}"> ${'selected'} </c:if>>30대</option>
											<option value="40"
												<c:if test="${edit.age == '40'}"> ${'selected'} </c:if>>40대</option>
											<option value="50"
												<c:if test="${edit.age == '50'}"> ${'selected'} </c:if>>50대이상</option>
									</select></td>
								</tr>

								<tr>
									<th>우편번호</th>
									<td><input name="zip" id="zip" size="5"
										value="${edit.zip}" class="input_box" readonly
										onclick="post_search()" /> <input type="button"
										value="우편번호검색" class="input_button"
								...
								<tr>
									...
									<td><input name="address1" id="address1" size="50"
										value="${edit.address1}" class="input_box" readonly
								...
								<tr>
									...
									<td><input name="address2" id="address2" size="37"
										value="${edit.address2}" class="input_box" /></td>
								</tr>

								<tr>
									<th>핸드폰번호</th>
									<td><input name="phone1" id="phone1" size="3"
										maxlength="3" value="${phone1}" class="input_box" />- <input
										name="phone2" id="phone2" size="4" maxlength="4"
										value="${phone2}" class="input_box" />- <input name="phone3"
										id="phone3" size="4" maxlength="4" value="${phone3}"
										class="input_box" /></td>
								</tr>
								<!-- <tr>
					<th>이메일</th>
						...
						<div id="emailcheck"></div></td>				
								...
								<tr>
									...
									<td><input name="mailid" id="mailid" size="10"
										value="${emailid}" class="input_box" /> @ <input
										name="maildomain" id="maildomain" size="20"
										value="${maildomain}" class="input_box" readonly /> <!-- readonly는 단지 쓰기,수정이 불가능하고 읽기만 가능하다  -->
							...
							<div id="join_menu">
								<input type="submit" value="확인" class="input_button" /> <input
